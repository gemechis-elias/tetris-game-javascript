...
function SetupCanvas(){
    ...
    // Draw controls text
    ctx.font = '19px Arial';
    ctx.fillText("↑ : Rotate Right", 310, 463);
...
function HandleKeyPress(key){
    ...
    // Left arrow keycode
    if(key.keyCode === 37){ // Previously 65 for 'a'
    ...
    // Right arrow keycode
    } else if(key.keyCode === 39){ // Previously 68 for 'd'
    ...
    // Down arrow keycode
    } else if(key.keyCode === 40){ // Previously 83 for 's'
        MoveTetrominoDown();
    // Up arrow keycode for rotation
    } else if(key.keyCode === 38){ // Change this to the desired key for rotating, previously 'e' with keycode 69
        RotateTetromino();
    }
    } 
}
